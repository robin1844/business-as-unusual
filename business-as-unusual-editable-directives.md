# Business as Unusual — Editable Directive List

Edit this file freely while reviewing the collection.

- Delete a whole directive section if you want that directive removed from the app.
- Rewrite the **Heading** or **Bit** directly in its section if you want different wording.
- Keep the **ID** unchanged; it is the stable identifier used by favourites and hidden directives.
- Headings should remain unique, preferably two to four words, and should avoid repeating prominent words from the bit.
- Add comments beneath a directive if you are unsure; they can be resolved during the next app update.

**Current collection:** 232 directives

---

## Directive 001

- **ID:** `directive-001`
- **Heading:** EXECUTIVE JUDGEMENT

**Bit**

Whenever someone presses a lift button—any button—say, “Excellent choice.”

---

## Directive 002

- **ID:** `directive-002`
- **Heading:** PREMIUM MOBILITY

**Bit**

When a lift arrives, gesture grandly towards it and say, “Your transport awaits.”

---

## Directive 003

- **ID:** `directive-003`
- **Heading:** POINT OF NO RETURN

**Bit**

As the lift doors close, quietly say, “And so it begins.”

---

## Directive 004

- **ID:** `directive-004`
- **Heading:** CONFIDENT MISDIRECTION

**Bit**

Whenever a lift reaches a floor, say, “This is us,” even if you aren’t getting out.

---

## Directive 005

- **ID:** `directive-005`
- **Heading:** FIELD DEPLOYMENT

**Bit**

As someone gets out of a lift, say, “Good luck out there.”

---

## Directive 006

- **ID:** `directive-006`
- **Heading:** STAKEHOLDER ENGAGEMENT

**Bit**

When leaving a completely silent lift, say, “Lovely talking to you all.”

---

## Directive 007

- **ID:** `directive-007`
- **Heading:** INSTITUTIONAL CONFIDENCE

**Bit**

When someone holds the lift for you, say, “You’ve restored my faith in the system.”

---

## Directive 008

- **ID:** `directive-008`
- **Heading:** HEADCOUNT VARIANCE

**Bit**

If an empty lift arrives, inspect it and say, “Perfect—the delegation hasn’t arrived.”

---

## Directive 009

- **ID:** `directive-009`
- **Heading:** LAUNCH EXCELLENCE

**Bit**

As a lift begins moving, nod approvingly and say, “Textbook departure.”

---

## Directive 010

- **ID:** `directive-010`
- **Heading:** EXCEPTION HANDLING

**Bit**

When lift doors reopen for a late arrival, announce, “We have a late entry!”

---

## Directive 011

- **ID:** `directive-011`
- **Heading:** PREMIUM AIR SERVICE

**Bit**

At the end of a lift journey, say, “Thank you for flying with us.”

---

## Directive 012

- **ID:** `directive-012`
- **Heading:** STRATEGIC ALIGNMENT

**Bit**

When someone selects the same floor as you, say, “Ah—a fellow strategist.”

---

## Directive 013

- **ID:** `directive-013`
- **Heading:** PREDICTIVE INFRASTRUCTURE

**Bit**

Whenever an automatic door opens, quietly say, “It knows.”

---

## Directive 014

- **ID:** `directive-014`
- **Heading:** LONG-TERM GRATITUDE

**Bit**

When someone holds a door for you, say, “I won’t forget this.”

---

## Directive 015

- **ID:** `directive-015`
- **Heading:** TREATY SUCCESS

**Bit**

When walking through an open door, say, “The access negotiations were successful.”

---

## Directive 016

- **ID:** `directive-016`
- **Heading:** CREDENTIAL CONFIDENCE

**Bit**

When someone uses their pass to let you through a secure door, say, “Your clearance checks out.”

---

## Directive 017

- **ID:** `directive-017`
- **Heading:** ORGANISATIONAL DIVERGENCE

**Bit**

When you and a colleague separate at a corridor junction, say, “We part ways here.”

---

## Directive 018

- **ID:** `directive-018`
- **Heading:** COORDINATED MOVEMENT

**Bit**

When someone is walking in the same direction as you, say, “Good—we’re forming a convoy.”

---

## Directive 019

- **ID:** `directive-019`
- **Heading:** ADVANCE PARTY

**Bit**

When overtaking someone in a corridor, say, “I’ll scout ahead.”

---

## Directive 020

- **ID:** `directive-020`
- **Heading:** CAPACITY COLLISION

**Bit**

When two people nearly collide in a doorway, say, “Classic stakeholder congestion.”

---

## Directive 021

- **ID:** `directive-021`
- **Heading:** INFLUENCE NETWORK

**Bit**

Whenever someone asks you to do a trivial task, say, “I should be able to pull some strings.”

---

## Directive 022

- **ID:** `directive-022`
- **Heading:** EXECUTIVE OWNERSHIP

**Bit**

When asked to move something a few inches, say, “Leave it with me.”

---

## Directive 023

- **ID:** `directive-023`
- **Heading:** INSIDER ADVANTAGE

**Bit**

Before using the photocopier, say, “Don’t worry—I know a person.”

---

## Directive 024

- **ID:** `directive-024`
- **Heading:** DELIVERY EXCELLENCE

**Bit**

Before attaching a document to an email, say, “Let me work my magic.”

---

## Directive 025

- **ID:** `directive-025`
- **Heading:** FEASIBILITY REVIEW

**Bit**

When asked to close a window, say, “I’ll see what I can do.”

---

## Directive 026

- **ID:** `directive-026`
- **Heading:** SENIOR CONTACTS

**Bit**

Before switching on a light, say, “I can make some calls.”

---

## Directive 027

- **ID:** `directive-027`
- **Heading:** SERVICE COMMITMENT

**Bit**

When asked to pass someone a pen, say, “Consider it handled.”

---

## Directive 028

- **ID:** `directive-028`
- **Heading:** UPWARD MOBILITY

**Bit**

When asked to put something on a shelf, say, “I’ll take it upstairs.”

---

## Directive 029

- **ID:** `directive-029`
- **Heading:** MODERN CAPABILITY

**Bit**

Before using a stapler, say, “We have the technology.”

---

## Directive 030

- **ID:** `directive-030`
- **Heading:** CRISIS EXPERIENCE

**Bit**

Before clearing a paper jam, say, “I’ve dealt with this kind of situation before.”

---

## Directive 031

- **ID:** `directive-031`
- **Heading:** OPERATIONAL DIFFICULTY

**Bit**

When asked to forward an email, sigh and say, “It won’t be easy.”

---

## Directive 032

- **ID:** `directive-032`
- **Heading:** DELIVERY ESTIMATE

**Bit**

Before changing a filename, say, “I’m going to need a few minutes.”

---

## Directive 033

- **ID:** `directive-033`
- **Heading:** RELATIONSHIP CAPITAL

**Bit**

Before opening Outlook, say, “Fortunately, I have contacts.”

---

## Directive 034

- **ID:** `directive-034`
- **Heading:** PRIORITY HANDLING

**Bit**

Immediately after pressing Send, say, “I’ve expedited it.”

---

## Directive 035

- **ID:** `directive-035`
- **Heading:** RAPID MOBILISATION

**Bit**

When walking towards the kettle, say, “The wheels are already in motion.”

---

## Directive 036

- **ID:** `directive-036`
- **Heading:** GOVERNANCE COMPLETE

**Bit**

After borrowing a chair, say, “I’ve secured the necessary approvals.”

---

## Directive 037

- **ID:** `directive-037`
- **Heading:** DELIVERY MILESTONE

**Bit**

After successfully opening stubborn packaging, say, “We got it over the line.”

---

## Directive 038

- **ID:** `directive-038`
- **Heading:** SERVICE RECOVERY

**Bit**

After replacing the printer paper, say, “There were complications, but we prevailed.”

---

## Directive 039

- **ID:** `directive-039`
- **Heading:** BUSINESS CONTINUITY

**Bit**

After fixing something with sticky tape, say, “I’ve deployed a workaround.”

---

## Directive 040

- **ID:** `directive-040`
- **Heading:** RESTRICTED CIRCULATION

**Bit**

Before sharing a completely ordinary spreadsheet, say, “Let’s keep this between ourselves.”

---

## Directive 041

- **ID:** `directive-041`
- **Heading:** FULL READINESS

**Bit**

When everyone has arrived for a meeting, say, “The pieces are in position.”

---

## Directive 042

- **ID:** `directive-042`
- **Heading:** STAKEHOLDER COURTESY

**Bit**

At the start of an informal conversation, say, “Thanks for making the time.”

---

## Directive 043

- **ID:** `directive-043`
- **Heading:** CRITICAL MASS

**Bit**

When the final person joins a meeting, say, “Excellent—we’re quorate.”

---

## Directive 044

- **ID:** `directive-044`
- **Heading:** POWER POSITIONING

**Bit**

Before sitting down in a meeting room, ask, “Is this the strategic end of the table?”

---

## Directive 045

- **ID:** `directive-045`
- **Heading:** CONFIDENTIAL SESSION

**Bit**

When someone closes the meeting-room door, say, “Good. Now we can speak freely.”

---

## Directive 046

- **ID:** `directive-046`
- **Heading:** EVIDENCE SECURED

**Bit**

When someone begins sharing their screen, say, “We have visual confirmation.”

---

## Directive 047

- **ID:** `directive-047`
- **Heading:** MILESTONE RECOGNITION

**Bit**

Whenever a presentation advances to a new slide, quietly say, “There it is.”

---

## Directive 048

- **ID:** `directive-048`
- **Heading:** STRATEGIC REVELATION

**Bit**

When someone makes an extremely obvious point, nod gravely and say, “That changes everything.”

---

## Directive 049

- **ID:** `directive-049`
- **Heading:** MULTICHANNEL CONFIRMATION

**Bit**

When asked whether you can see someone’s screen, reply, “Loud and clear.”

---

## Directive 050

- **ID:** `directive-050`
- **Heading:** QUALITY BENCHMARK

**Bit**

Whenever someone draws a rough circle on a whiteboard, say, “That’s a very strong circle.”

---

## Directive 051

- **ID:** `directive-051`
- **Heading:** EFFICIENCY DIVIDEND

**Bit**

When a meeting finishes early, say, “An unprecedented operational success.”

---

## Directive 052

- **ID:** `directive-052`
- **Heading:** ACCEPTABLE LOSSES

**Bit**

When a meeting runs late, say, “We always knew there would be casualties.”

---

## Directive 053

- **ID:** `directive-053`
- **Heading:** IMPLICIT CONSENSUS

**Bit**

When nobody has any questions, say, “The silence speaks volumes.”

---

## Directive 054

- **ID:** `directive-054`
- **Heading:** DEFERRED ACTION

**Bit**

When someone says, “Let’s take that offline,” look impressed and write it down.

---

## Directive 055

- **ID:** `directive-055`
- **Heading:** SECONDARY OPERATIONS

**Bit**

When refreshments arrive during a meeting, say, “Excellent—the second phase.”

---

## Directive 056

- **ID:** `directive-056`
- **Heading:** RISK ESCALATION

**Bit**

Introduce a completely ordinary agenda item by saying, “Now we reach the difficult part.”

---

## Directive 057

- **ID:** `directive-057`
- **Heading:** CONTINUITY MEASURES

**Bit**

When someone temporarily leaves a meeting, say, “We’ll continue in their absence.”

---

## Directive 058

- **ID:** `directive-058`
- **Heading:** SHARED UNDERSTANDING

**Bit**

At the end of an inconclusive meeting, say, “I think we all know what we have to do.”

---

## Directive 059

- **ID:** `directive-059`
- **Heading:** FORMAL RESOLUTION

**Bit**

When closing your notebook at the end of a meeting, say, “Then it’s settled.”

---

## Directive 060

- **ID:** `directive-060`
- **Heading:** DUTY RECOGNITION

**Bit**

As people leave a meeting, say, “Thank you for your service.”

---

## Directive 061

- **ID:** `directive-061`
- **Heading:** CHANNEL OPEN

**Bit**

When someone unmutes during a video call, say, “We have you.”

---

## Directive 062

- **ID:** `directive-062`
- **Heading:** COMMS RESTORED

**Bit**

When someone on a video call asks, “Can you hear me?” reply, “We can now.”

---

## Directive 063

- **ID:** `directive-063`
- **Heading:** IDENTITY CONFIRMED

**Bit**

When someone’s camera switches on, say, “Visuals restored.”

---

## Directive 064

- **ID:** `directive-064`
- **Heading:** CONNECTION CASUALTY

**Bit**

When someone freezes during a video call, say, “We’ve lost them.”

---

## Directive 065

- **ID:** `directive-065`
- **Heading:** LONG-TERM RECOVERY

**Bit**

When a frozen colleague begins moving again, say, “Welcome back. It’s been years.”

---

## Directive 066

- **ID:** `directive-066`
- **Heading:** BANDWIDTH CONFLICT

**Bit**

When two people speak simultaneously on a video call, say, “Cross-channel interference.”

---

## Directive 067

- **ID:** `directive-067`
- **Heading:** TRANSPARENCY EVENT

**Bit**

When someone accidentally shares the wrong window, say, “Interesting—very revealing.”

---

## Directive 068

- **ID:** `directive-068`
- **Heading:** RESILIENCE PROVEN

**Bit**

When screen sharing works on the first attempt, say, “The technology has held.”

---

## Directive 069

- **ID:** `directive-069`
- **Heading:** IDENTITY SAFEGUARD

**Bit**

When someone’s background blur behaves strangely, ask, “Are you entering the witness protection programme?”

---

## Directive 070

- **ID:** `directive-070`
- **Heading:** WELFARE CONCERN

**Bit**

When someone joins a video call late, say, “We were beginning to fear the worst.”

---

## Directive 071

- **ID:** `directive-071`
- **Heading:** EXTERNAL ADVISORY

**Bit**

When a pet appears during a video call, say, “I see you’ve brought legal counsel.”

---

## Directive 072

- **ID:** `directive-072`
- **Heading:** EMERGING TALENT

**Bit**

When a child appears during a video call, say, “Our newest stakeholder has joined us.”

---

## Directive 073

- **ID:** `directive-073`
- **Heading:** UNRECORDED CONTRIBUTION

**Bit**

When someone realises they have been speaking on mute, say, “We heard the entire speech in silence.”

---

## Directive 074

- **ID:** `directive-074`
- **Heading:** CEREMONIAL CLOSURE

**Bit**

When everyone waves goodbye at the end of a video call, perform an unnecessarily solemn salute.

---

## Directive 075

- **ID:** `directive-075`
- **Heading:** PRIVATE DEBRIEF

**Bit**

When you are the last person left on a video call, wait two seconds and say, “Finally.”

---

## Directive 076

- **ID:** `directive-076`
- **Heading:** COURTESY CONTINUITY

**Bit**

Whenever an email exchange reaches “Thanks,” reply with “No problem” and keep the ceremony going for as long as possible.

---

## Directive 077

- **ID:** `directive-077`
- **Heading:** EXECUTIVE CORRESPONDENCE

**Bit**

End a routine email with, “Yours in operational readiness.”

---

## Directive 078

- **ID:** `directive-078`
- **Heading:** EVIDENCE-LED OPENING

**Bit**

Begin a one-line email with, “Following extensive internal consultation…”

---

## Directive 079

- **ID:** `directive-079`
- **Heading:** RESTRICTED EVIDENCE

**Bit**

Whenever attaching an ordinary file, refer to it as “the supporting dossier.”

---

## Directive 080

- **ID:** `directive-080`
- **Heading:** AUDIT AMENDMENT

**Bit**

After correcting a typo, announce, “The record has been amended.”

---

## Directive 081

- **ID:** `directive-081`
- **Heading:** PASSIVE BRIEFING

**Bit**

When forwarding an unimportant email, write, “For situational awareness.”

---

## Directive 082

- **ID:** `directive-082`
- **Heading:** DECISION ARCHITECTURE

**Bit**

When sending someone a menu, write, “Please find the available pathways attached.”

---

## Directive 083

- **ID:** `directive-083`
- **Heading:** MISSION CONTROL

**Bit**

Whenever referring to the workplace group chat, call it “the command channel.”

---

## Directive 084

- **ID:** `directive-084`
- **Heading:** URGENT ESCALATION

**Bit**

When someone posts a photograph of their lunch, reply, “This has been escalated.”

---

## Directive 085

- **ID:** `directive-085`
- **Heading:** SUPPORTING MATERIAL

**Bit**

When someone sends a GIF in a work chat, reply, “Useful context, thank you.”

---

## Directive 086

- **ID:** `directive-086`
- **Heading:** POSITIVE FORECAST

**Bit**

When asked a simple yes-or-no question, reply, “All indicators point to yes.”

---

## Directive 087

- **ID:** `directive-087`
- **Heading:** STEADY-STATE OPERATIONS

**Bit**

When someone gives you an ordinary update, reply, “Understood. Maintain course.”

---

## Directive 088

- **ID:** `directive-088`
- **Heading:** MATTER CONCLUDED

**Bit**

After confirming the office milk order, say, “No further action is required at this time.”

---

## Directive 089

- **ID:** `directive-089`
- **Heading:** MANDATORY ATTENDANCE

**Bit**

Whenever someone sends a calendar invitation, refer to it as “the formal summons.”

---

## Directive 090

- **ID:** `directive-090`
- **Heading:** PRIORITY INVERSION

**Bit**

Mark a thoroughly unimportant message, “For immediate de-prioritisation.”

---

## Directive 091

- **ID:** `directive-091`
- **Heading:** CONDUCT REMINDER

**Bit**

Finish a routine workplace announcement with, “Please govern yourselves accordingly.”

---

## Directive 092

- **ID:** `directive-092`
- **Heading:** SCOPE MISALIGNMENT

**Bit**

When someone includes you in an email by mistake, reply, “I believe I have been brought into this operation prematurely.”

---

## Directive 093

- **ID:** `directive-093`
- **Heading:** CLEAN EXIT

**Bit**

When removed from an email thread, announce, “My involvement in the matter has concluded.”

---

## Directive 094

- **ID:** `directive-094`
- **Heading:** MEDIA ALERT

**Bit**

Write “BREAKING” in the group chat before announcing that biscuits have arrived.

---

## Directive 095

- **ID:** `directive-095`
- **Heading:** FORMAL NOTICE

**Bit**

After posting an ordinary message in Teams or Slack, say, “I’ve issued a communiqué.”

---

## Directive 096

- **ID:** `directive-096`
- **Heading:** HUMAN CAPITAL STABILISATION

**Bit**

When someone makes tea, say, “Good. Morale was becoming an issue.”

---

## Directive 097

- **ID:** `directive-097`
- **Heading:** OUTPUT RESTORED

**Bit**

When you discover fresh coffee, announce, “Production has resumed.”

---

## Directive 098

- **ID:** `directive-098`
- **Heading:** TARGET CONDITION

**Bit**

When the kettle boils, say, “We have achieved temperature.”

---

## Directive 099

- **ID:** `directive-099`
- **Heading:** POLICY REVIEW

**Bit**

When someone offers you a biscuit, ask, “What’s the governance around these?”

---

## Directive 100

- **ID:** `directive-100`
- **Heading:** EXECUTIVE ASSORTMENT

**Bit**

When presented with a selection of biscuits, inspect them and say, “A strong portfolio.”

---

## Directive 101

- **ID:** `directive-101`
- **Heading:** SUPPLY DISRUPTION

**Bit**

When the milk carton is empty, announce, “We have a supply-chain event.”

---

## Directive 102

- **ID:** `directive-102`
- **Heading:** SUPPLY NORMALISATION

**Bit**

When fresh milk arrives, announce, “Business continuity has been restored.”

---

## Directive 103

- **ID:** `directive-103`
- **Heading:** ASSET DISPOSAL

**Bit**

When looking into an empty office fridge, say, “They’ve cleared the site.”

---

## Directive 104

- **ID:** `directive-104`
- **Heading:** UNEXPECTED BENEFIT

**Bit**

When someone brings in cake, ask, “Who authorised this—and how do we thank them?”

---

## Directive 105

- **ID:** `directive-105`
- **Heading:** ATTENDANCE CHECK

**Bit**

Before cutting a cake, ask, “Are all key stakeholders present?”

---

## Directive 106

- **ID:** `directive-106`
- **Heading:** PROCUREMENT QUALITY

**Bit**

When someone chooses a mug, say, “That’s one of our stronger vessels.”

---

## Directive 107

- **ID:** `directive-107`
- **Heading:** COST-EFFECTIVE DELIVERY

**Bit**

After making instant coffee, say, “It’s not glamorous, but it’s scalable.”

---

## Directive 108

- **ID:** `directive-108`
- **Heading:** TIMELY DELIVERY

**Bit**

When toast pops out of the toaster, say, “Right on schedule.”

---

## Directive 109

- **ID:** `directive-109`
- **Heading:** PERIMETER BREACH

**Bit**

When someone spills a tiny amount of water, announce, “Containment has failed.”

---

## Directive 110

- **ID:** `directive-110`
- **Heading:** MATTER RESOLVED

**Bit**

After wiping up a small spill, say, “The incident is now closed.”

---

## Directive 111

- **ID:** `directive-111`
- **Heading:** CORE OUTPUT

**Bit**

Whenever discussing lunch, refer to it as “the midday deliverable.”

---

## Directive 112

- **ID:** `directive-112`
- **Heading:** FIELD REVIEW

**Bit**

When someone returns from lunch, ask, “Was the mission successful?”

---

## Directive 113

- **ID:** `directive-113`
- **Heading:** SPECIALIST SUPPORT

**Bit**

When a takeaway delivery arrives, announce, “The external consultants are here.”

---

## Directive 114

- **ID:** `directive-114`
- **Heading:** EXECUTIVE HOTLINE

**Bit**

Hold a banana to your ear and say, “I’m afraid I have to take this.”

---

## Directive 115

- **ID:** `directive-115`
- **Heading:** VALUES ALIGNMENT

**Bit**

Occasionally end a mundane instruction with “Amen,” then continue as though nothing unusual happened.

---

## Directive 116

- **ID:** `directive-116`
- **Heading:** EXCEPTIONAL PERFORMANCE

**Bit**

When the printer works on the first attempt, say, “We are witnessing history.”

---

## Directive 117

- **ID:** `directive-117`
- **Heading:** APPROVAL WITHHELD

**Bit**

When the printer fails, say, “It has rejected the proposal.”

---

## Directive 118

- **ID:** `directive-118`
- **Heading:** SYSTEM APPEASEMENT

**Bit**

Before pressing Print, say, “Let’s see if it accepts the offering.”

---

## Directive 119

- **ID:** `directive-119`
- **Heading:** AUTHORISED OUTPUT

**Bit**

When a printed page emerges, announce, “The documents have been released.”

---

## Directive 120

- **ID:** `directive-120`
- **Heading:** SPECIALIST INTERVENTION

**Bit**

When someone clears a printer jam, address them as “Engineering.”

---

## Directive 121

- **ID:** `directive-121`
- **Heading:** INFORMATION GOVERNANCE

**Bit**

Whenever referring to the shredder, call it “Records Management.”

---

## Directive 122

- **ID:** `directive-122`
- **Heading:** STANDARDS COMPLIANCE

**Bit**

Before using a stapler, ask, “Is this the approved fastening solution?”

---

## Directive 123

- **ID:** `directive-123`
- **Heading:** CUSTODIAL RESPONSIBILITY

**Bit**

When handing someone a pen, say, “Use it wisely.”

---

## Directive 124

- **ID:** `directive-124`
- **Heading:** END-OF-SERVICE HONOURS

**Bit**

When a pen runs out, hold it solemnly and say, “It gave everything.”

---

## Directive 125

- **ID:** `directive-125`
- **Heading:** FITNESS FOR PURPOSE

**Bit**

After testing a pen, nod seriously and say, “It’s operational.”

---

## Directive 126

- **ID:** `directive-126`
- **Heading:** ASSET REPATRIATION

**Bit**

When someone borrows your scissors, say, “Bring them home safely.”

---

## Directive 127

- **ID:** `directive-127`
- **Heading:** ENTERPRISE SPECIFICATION

**Bit**

Whenever you see an unusually large paperclip, describe it as “industrial grade.”

---

## Directive 128

- **ID:** `directive-128`
- **Heading:** POWER DEPENDENCY

**Bit**

When plugging in a laptop, say, “Commencing life support.”

---

## Directive 129

- **ID:** `directive-129`
- **Heading:** INFRASTRUCTURE RECOVERY

**Bit**

When the Wi-Fi reconnects, announce, “We’re back on the grid.”

---

## Directive 130

- **ID:** `directive-130`
- **Heading:** MISSION-CRITICAL SUPPORT

**Bit**

When an IT colleague fixes a minor problem, say, “You’ve saved more than you know.”

---

## Directive 131

- **ID:** `directive-131`
- **Heading:** LEGACY METHODOLOGY

**Bit**

When turning something off and on fixes it, say, “The ancient protocol remains effective.”

---

## Directive 132

- **ID:** `directive-132`
- **Heading:** INNOVATION TRIAL

**Bit**

Put a removable “VOICE ACTIVATED” sign on a suitable, non-critical office appliance.

---

## Directive 133

- **ID:** `directive-133`
- **Heading:** SYSTEM READINESS

**Bit**

Put a removable note on a keyboard saying, “Please wait—keyboard loading.”

---

## Directive 134

- **ID:** `directive-134`
- **Heading:** COMPATIBILITY REVIEW

**Bit**

When someone asks to borrow a charger, lower your voice and ask, “What connector?”

---

## Directive 135

- **ID:** `directive-135`
- **Heading:** STRATEGIC CONNECTION

**Bit**

Whenever plugging in an HDMI cable, refer to it as “the final link.”

---

## Directive 136

- **ID:** `directive-136`
- **Heading:** RESOURCE LOCATED

**Bit**

When someone arrives at work, look relieved and say, “There they are.”

---

## Directive 137

- **ID:** `directive-137`
- **Heading:** TIMING EXCELLENCE

**Bit**

When someone arrives exactly on time, look at the clock and say, “Impeccable.”

---

## Directive 138

- **ID:** `directive-138`
- **Heading:** COMPLIANCE REMINDER

**Bit**

When someone arrives one minute late, silently tap your watch.

---

## Directive 139

- **ID:** `directive-139`
- **Heading:** INTERIM STEWARDSHIP

**Bit**

When someone returns from the toilet, say, “Welcome back. We covered for you.”

---

## Directive 140

- **ID:** `directive-140`
- **Heading:** EXIT INDICATORS

**Bit**

Whenever someone puts on their coat, ask, “Going somewhere?”

---

## Directive 141

- **ID:** `directive-141`
- **Heading:** ARCHIVAL MILESTONE

**Bit**

At the end of the working day, say, “Another one for the history books.”

---

## Directive 142

- **ID:** `directive-142`
- **Heading:** ESCAPE WINDOW

**Bit**

When someone leaves work early, say, “Get out while you still can.”

---

## Directive 143

- **ID:** `directive-143`
- **Heading:** SKELETON OPERATIONS

**Bit**

When only you and one colleague remain in the office, say, “We’re the last line of defence.”

---

## Directive 144

- **ID:** `directive-144`
- **Heading:** COMPULSORY RETURN

**Bit**

On Monday morning, say, “I see we’ve all been recalled.”

---

## Directive 145

- **ID:** `directive-145`
- **Heading:** WEEKEND TRANSITION

**Bit**

On Friday afternoon, announce, “Extraction is imminent.”

---

## Directive 146

- **ID:** `directive-146`
- **Heading:** BUSINESS RESUMPTION

**Bit**

When someone returns from holiday, say, “Your desk has been preserved exactly as you left it.”

---

## Directive 147

- **ID:** `directive-147`
- **Heading:** MISSION HOMECOMING

**Bit**

Welcome a colleague back from lunch as though they have returned from a dangerous expedition.

---

## Directive 148

- **ID:** `directive-148`
- **Heading:** CASE LOGGED

**Bit**

When someone sneezes once, say, “Noted.”

---

## Directive 149

- **ID:** `directive-149`
- **Heading:** RECURRING SIGNAL MANAGEMENT

**Bit**

When someone sitting near you sneezes more than once, say, “We heard you the first time.”

---

## Directive 150

- **ID:** `directive-150`
- **Heading:** BRAND ALIGNMENT

**Bit**

When three colleagues wear the same colour, announce, “Good—the uniform guidance reached everyone.”

---

## Directive 151

- **ID:** `directive-151`
- **Heading:** PORTFOLIO SELECTION

**Bit**

When someone chooses a sandwich, describe it as “a strategic procurement decision.”

---

## Directive 152

- **ID:** `directive-152`
- **Heading:** STRUCTURAL REALIGNMENT

**Bit**

When people change desks, describe it as “an organisational transformation.”

---

## Directive 153

- **ID:** `directive-153`
- **Heading:** CLIMATE STRATEGY

**Bit**

When someone opens a window, call it “an environmental intervention.”

---

## Directive 154

- **ID:** `directive-154`
- **Heading:** CAPACITY OPTIMISATION

**Bit**

When someone changes seats, call it “a resource reallocation.”

---

## Directive 155

- **ID:** `directive-155`
- **Heading:** SYNERGY ACTIVATION

**Bit**

When tidying the office kitchen, announce, “This is a cross-functional initiative.”

---

## Directive 156

- **ID:** `directive-156`
- **Heading:** FALLBACK ROUTE

**Bit**

When taking the stairs instead of the lift, say, “We’re using the manual contingency.”

---

## Directive 157

- **ID:** `directive-157`
- **Heading:** PERIMETER REVIEW

**Bit**

When going for a walk around the block, announce, “I’m heading off-site.”

---

## Directive 158

- **ID:** `directive-158`
- **Heading:** UNOFFICIAL BRIEFING

**Bit**

Whenever someone shares office gossip, describe it as “informal market intelligence.”

---

## Directive 159

- **ID:** `directive-159`
- **Heading:** RESIDUAL VALUE

**Bit**

When discussing leftovers, refer to them as “legacy assets.”

---

## Directive 160

- **ID:** `directive-160`
- **Heading:** TEMPORARY ALLIANCE

**Bit**

When borrowing a pen, describe the arrangement as “a short-term equipment partnership.”

---

## Directive 161

- **ID:** `directive-161`
- **Heading:** LOSS PREVENTION

**Bit**

When someone loses a borrowed pen, call it “an asset-tracking discrepancy.”

---

## Directive 162

- **ID:** `directive-162`
- **Heading:** INFORMAL STANDARDS

**Bit**

On casual Friday, announce, “The relaxed governance window is now open.”

---

## Directive 163

- **ID:** `directive-163`
- **Heading:** SERVICEABILITY CONCERN

**Bit**

When discovering a broken chair, announce, “We have a seating continuity risk.”

---

## Directive 164

- **ID:** `directive-164`
- **Heading:** CLIMATE ADJUSTMENT

**Bit**

When turning up the heating, say, “We’re implementing a thermal strategy pivot.”

---

## Directive 165

- **ID:** `directive-165`
- **Heading:** EXTERNAL PROVISION

**Bit**

When ordering pizza, say, “We’ve decided to outsource.”

---

## Directive 166

- **ID:** `directive-166`
- **Heading:** OPPORTUNISTIC GROWTH

**Bit**

When accepting someone’s unwanted chips, describe it as “an acquisition.”

---

## Directive 167

- **ID:** `directive-167`
- **Heading:** PREVENTIVE SERVICING

**Bit**

When washing a mug, say, “Just carrying out some routine asset maintenance.”

---

## Directive 168

- **ID:** `directive-168`
- **Heading:** SUSTAINABILITY PROGRAMME

**Bit**

When moving an office plant, call it “a green infrastructure project.”

---

## Directive 169

- **ID:** `directive-169`
- **Heading:** SITE SELECTION

**Bit**

When deciding where to have lunch, say, “We need to agree our location strategy.”

---

## Directive 170

- **ID:** `directive-170`
- **Heading:** SEASONAL GOVERNANCE

**Bit**

Whenever discussing the office Christmas party, call it “the annual stakeholder summit.”

---

## Directive 171

- **ID:** `directive-171`
- **Heading:** LEADERSHIP APPOINTMENT

**Bit**

Give the office plant a formal job title and occasionally ask for its opinion.

---

## Directive 172

- **ID:** `directive-172`
- **Heading:** REGIONAL EXPANSION

**Bit**

Choose an underused meeting room and refer to it exclusively as “the northern facility.”

---

## Directive 173

- **ID:** `directive-173`
- **Heading:** PERFORMANCE METRICS

**Bit**

Maintain a completely serious league table for trivial workplace achievements.

---

## Directive 174

- **ID:** `directive-174`
- **Heading:** INSTANT RECOGNITION

**Bit**

Award someone “Employee of the Minute” for a tiny act of competence.

---

## Directive 175

- **ID:** `directive-175`
- **Heading:** CULTURAL SYMBOL

**Bit**

Select an ordinary office object as the department’s ceremonial artefact.

---

## Directive 176

- **ID:** `directive-176`
- **Heading:** ENDWEEK FRAMEWORK

**Bit**

Every Friday morning, announce, “Casual protocols are now in effect.”

---

## Directive 177

- **ID:** `directive-177`
- **Heading:** FAILURE TAXONOMY

**Bit**

Give every recurring printer fault an official incident name.

---

## Directive 178

- **ID:** `directive-178`
- **Heading:** MORALE MONITORING

**Bit**

Maintain a “Days Since Last Biscuit” counter.

---

## Directive 179

- **ID:** `directive-179`
- **Heading:** QUARTERLY APPRAISAL

**Bit**

Conduct a formal performance review of the temperamental office kettle.

---

## Directive 180

- **ID:** `directive-180`
- **Heading:** HONOURABLE DISCHARGE

**Bit**

Hold a ten-second retirement ceremony whenever a pen runs out.

---

## Directive 181

- **ID:** `directive-181`
- **Heading:** APPROVED PROVIDER

**Bit**

Designate one mug brand as “the organisation’s preferred vessel supplier.”

---

## Directive 182

- **ID:** `directive-182`
- **Heading:** LEADERSHIP ACCOMMODATION

**Bit**

Choose one ordinary office chair and refer to it as “the executive suite.”

---

## Directive 183

- **ID:** `directive-183`
- **Heading:** CAREER ACCELERATION

**Bit**

Whenever someone fixes a minor technical problem, immediately promote them.

---

## Directive 184

- **ID:** `directive-184`
- **Heading:** STRATEGIC INFLUENCE

**Bit**

Introduce a colleague as “the person behind the person.”

---

## Directive 185

- **ID:** `directive-185`
- **Heading:** RECOGNITION CYCLE

**Bit**

Refer to whoever most recently made tea as “this quarter’s outstanding contributor.”

---

## Directive 186

- **ID:** `directive-186`
- **Heading:** EXECUTIVE RESEARCH

**Bit**

Carry a clipboard containing nothing except strongly held lunch opinions.

---

## Directive 187

- **ID:** `directive-187`
- **Heading:** AUTHORISATION GATE

**Bit**

Ask for verbal sign-off before opening a new packet of biscuits.

---

## Directive 188

- **ID:** `directive-188`
- **Heading:** LOGISTICS HUB

**Bit**

Refer to the stationery cupboard exclusively as “Central Stores.”

---

## Directive 189

- **ID:** `directive-189`
- **Heading:** CUSTODY ZONE

**Bit**

Refer to the coat rack exclusively as “Personal Effects.”

---

## Directive 190

- **ID:** `directive-190`
- **Heading:** MARKET COMPETITION

**Bit**

Establish a completely fictional rivalry between two photocopiers.

---

## Directive 191

- **ID:** `directive-191`
- **Heading:** SECURE DISPATCH

**Bit**

Treat every arrival of post as though it were a diplomatic pouch.

---

## Directive 192

- **ID:** `directive-192`
- **Heading:** MEDIA RELATIONS

**Bit**

Hold a one-question press conference whenever someone returns from an errand.

---

## Directive 193

- **ID:** `directive-193`
- **Heading:** PROGRAMME BRANDING

**Bit**

Give every mundane office task an unnecessarily dramatic project name.

---

## Directive 194

- **ID:** `directive-194`
- **Heading:** POLICY FINALISATION

**Bit**

After explaining a routine process, finish with, “And that is the doctrine.”

---

## Directive 195

- **ID:** `directive-195`
- **Heading:** INFORMATION SECURITY

**Bit**

Give a trivial task a secret code name and politely refuse to explain it.

---

## Directive 196

- **ID:** `directive-196`
- **Heading:** LAUNCH APPROVAL

**Bit**

Before starting the microwave, ask, “Are we cleared to proceed?”

---

## Directive 197

- **ID:** `directive-197`
- **Heading:** INFRASTRUCTURE APPRECIATION

**Bit**

Whenever automatic lights switch on, look upwards and say, “Thank you.”

---

## Directive 198

- **ID:** `directive-198`
- **Heading:** DELIVERY RECOGNITION

**Bit**

When the microwave finishes, pat it gently and say, “Outstanding work.”

---

## Directive 199

- **ID:** `directive-199`
- **Heading:** RELATIONSHIP MANAGEMENT

**Bit**

After successfully printing something, look at the printer and say, “Same time tomorrow.”

---

## Directive 200

- **ID:** `directive-200`
- **Heading:** CAPACITY EXHAUSTED

**Bit**

At precisely 5 p.m., close your laptop and say, “They can’t ask any more of us.”

---

## Directive 201

- **ID:** `directive-201`
- **Heading:** FLUENCY RECOGNITION

**Bit**

After a colleague stumbles over their words, say, “Couldn’t have said it better myself.”

---

## Directive 202

- **ID:** `directive-202`
- **Heading:** LITERARY BENCHMARK

**Bit**

After a colleague gets tongue-tied, say, “And they said Shakespeare is dead.”

---

## Directive 203

- **ID:** `directive-203`
- **Heading:** COMMUNICATION EXCELLENCE

**Bit**

After a colleague struggles to get their words out, say, “You just have such a way with words.”

---

## Directive 204

- **ID:** `directive-204`
- **Heading:** DELIVERY COMPLEXITY

**Bit**

After a colleague trips over a sentence, say, “That’s easy for you to say.”

---

## Directive 205

- **ID:** `directive-205`
- **Heading:** RECOVERY OPERATION

**Bit**

When someone arrives late at the office, say, “Cancel the search party.”

---

## Directive 206

- **ID:** `directive-206`
- **Heading:** SYSTEMS HANDOVER

**Bit**

When helping at someone else’s desk and asking to take control of their computer, say, “Mind if I drive?”

---

## Directive 207

- **ID:** `directive-207`
- **Heading:** ACTING LEADERSHIP

**Bit**

When leaving your desk to go to the toilet, tell a colleague, “You’re in charge while I’m gone.”

---

## Directive 208

- **ID:** `directive-208`
- **Heading:** SUCCESSION PLANNING

**Bit**

When leaving your desk to go to the toilet, tell a colleague, “If I don’t make it back, tell my story.”

---

## Directive 209

- **ID:** `directive-209`
- **Heading:** BACKGROUND CASTING

**Bit**

When someone asks whether you have any questions in a meeting, say, “I don’t have any speaking lines in this scene.”

---

## Directive 210

- **ID:** `directive-210`
- **Heading:** COMPOSURE MAINTAINED

**Bit**

Immediately after completely losing your cool over something, say, “Whoa, almost lost my cool there.”

---

## Directive 211

- **ID:** `directive-211`
- **Heading:** SUBJECT-MATTER EXPERTISE

**Bit**

When asked to complete a very straightforward task, say, “Boy, did you come to the right person.”

---

## Directive 212

- **ID:** `directive-212`
- **Heading:** EXECUTIVE DISCRETION

**Bit**

When asked to complete a very straightforward task, sigh and say, “Only because it’s you.”

---

## Directive 213

- **ID:** `directive-213`
- **Heading:** SEASONAL SANCTIONS

**Bit**

After any minor inconvenience at work, say, “That’s it—Christmas is cancelled.” This is especially effective nowhere near Christmas.

---

## Directive 214

- **ID:** `directive-214`
- **Heading:** RISK APPETITE

**Bit**

Whenever a colleague offers you something such as tea or coffee, say, “Why the hell not? I’m not driving.”

---

## Directive 215

- **ID:** `directive-215`
- **Heading:** TALENT MARKET SIGNAL

**Bit**

When a colleague arrives looking slightly smarter than usual, ask, “So, what time is your interview?”

---

## Directive 216

- **ID:** `directive-216`
- **Heading:** SHARED ACCOUNTABILITY

**Bit**

Whenever something was entirely your fault, say, “I can’t help but feel partially responsible.”

---

## Directive 217

- **ID:** `directive-217`
- **Heading:** PROCEEDINGS CONCLUDED

**Bit**

Immediately after asking a question in a meeting, say, “No further questions, Your Honour.”

---

## Directive 218

- **ID:** `directive-218`
- **Heading:** SAFE EXPERIMENTATION

**Bit**

After offering the first idea in a brainstorming session, say, “That’s great—let’s get the bad ideas out of the way first.”

---

## Directive 219

- **ID:** `directive-219`
- **Heading:** JUDICIAL ACCESS

**Bit**

Whenever you need to speak to someone at their desk, ask, “Permission to approach the bench?”

---

## Directive 220

- **ID:** `directive-220`
- **Heading:** PUBLIC RELATIONS

**Bit**

Whenever you are asked to sign something at work, say, “Anything for a fan.”

---

## Directive 221

- **ID:** `directive-221`
- **Heading:** ATTENDANCE CLASSIFICATION

**Bit**

If anyone leaves the office even a minute before 5 p.m., ask, “Half day, is it?”

---

## Directive 222

- **ID:** `directive-222`
- **Heading:** ALIGNMENT FAILURE

**Bit**

When a colleague says something you disagree with, say, “I’d love to agree with you, but then we’d both be wrong.”

---

## Directive 223

- **ID:** `directive-223`
- **Heading:** MOTIVATION DISCLOSURE

**Bit**

When you arrive late at the office or a meeting, say, “Sorry I’m late—I didn’t want to come.”

---

## Directive 224

- **ID:** `directive-224`
- **Heading:** HABIT REDUCTION

**Bit**

When a colleague tries to hand you something, say, “No thanks, I’m trying to quit.”

---

## Directive 225

- **ID:** `directive-225`
- **Heading:** PLACEMENT FLEXIBILITY

**Bit**

When someone accidentally drops something, say, “Oh, yeah—just put that anywhere.”

---

## Directive 226

- **ID:** `directive-226`
- **Heading:** OPPORTUNISTIC ACQUISITION

**Bit**

When someone accidentally drops something, immediately pick it up and say, “Finders keepers.”

---

## Directive 227

- **ID:** `directive-227`
- **Heading:** STANDARDS VIOLATION

**Bit**

When someone accidentally drops something, say, “Oh, we actually don’t do that here.”

---

## Directive 228

- **ID:** `directive-228`
- **Heading:** RECURRING ENGAGEMENT

**Bit**

When helping someone pick up something they dropped, say, “We’ve got to stop meeting like this.”

---

## Directive 229

- **ID:** `directive-229`
- **Heading:** SERVICE-LEVEL REVIEW

**Bit**

When someone tells you to have a nice day, reply, “I’ll think about it.”

---

## Directive 230

- **ID:** `directive-230`
- **Heading:** BOUNDARY SETTING

**Bit**

When someone tells you to have a nice day, reply, “Don’t tell me what to do.”

---

## Directive 231

- **ID:** `directive-231`
- **Heading:** PROFESSIONAL INTERRUPTION

**Bit**

Whenever you need to speak to a co-worker, begin with, “Sorry to bother you at work…”

---

## Directive 232

- **ID:** `directive-232`
- **Heading:** EPISTEMIC CAUTION

**Bit**

When someone asks a question with an objectively correct answer, such as when today’s meeting starts, say, “There’s no way to know.”

---

